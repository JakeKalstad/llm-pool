...
use crate::{api::{self, ApiResponse}, s3, LLMServer, State};
use tide::{http::mime, Request};

pub async fn files(req: Request<State>) -> tide::Result {
    let state =  req.state();
    let file_objs =s3::list_files(state.s3_url.clone(), state.s3_user.clone(), state.s3_pass.clone(), state.s3_bucket.clone()).await?;
...
pub async fn home(req: Request<State>) -> tide::Result {
    ...
    let queued = api::scan_keys(&mut redis, queue_pattern).await.unwrap_or_default();
    for q in queued {
        ...
        let response: ApiResponse = serde_json::from_str(&response_data_json).expect("response json is valid");
        queued_records.push(response);
    }
    let prompts = api::scan_keys(&mut redis, prompt_pattern).await.unwrap_or_default();
    for p in prompts {
